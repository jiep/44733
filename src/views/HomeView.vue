<script setup lang="ts">
  ...
  const paddedNumber = computed(() => {
    return number.value.toString().padStart(5, "0")
  });
  ...
  async function onClick(number: any) {
    ...
    const response = await fetch(`.netlify/functions/number?number=${paddedNumber}`)
...
<template>
  <div class="overflow-y-auto">
    <div class="flex m-4 pt-0 items-center">
      <input type="number" min="0" max="99999" :onkeyup="setRightValue"
